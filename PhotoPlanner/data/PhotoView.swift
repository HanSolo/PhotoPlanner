...
public class PhotoView: Equatable, Hashable {
    var name        : String
    ...
    init(name: String, description: String, cameraPoint: MKMapPoint, motifPoint: MKMapPoint, camera: Camera, lens: Lens, focalLength: Double, aperture: Double, orientation: CameraOrientation, countryCode: String, mapRect: MKMapRect, tags: Int? = 0, equipment: Int? = 0, times: Int? = 0) {
        self.name        = name
    ...
    init(dictionary: Dictionary<String, String>, cameras: [Camera], lenses: [Lens]) throws {
        let viewData : PhotoViewData = try PhotoViewData(dictionary: dictionary)
        ...
        self.orientation = CameraOrientation(rawValue: viewData.orientation!)!
        self.countryCode = viewData.countryCode!
        ...
        self.times     = Int(viewData.times ?? "0") ?? 0
    }
    
    init(viewData: PhotoViewData) {
        self.name                         = viewData.name!
        self.description                  = viewData.description!
        self.cameraPoint                  = MKMapPoint(CLLocationCoordinate2D(latitude: CLLocationDegrees((viewData.cameraLat! as NSString).doubleValue), longitude: CLLocationDegrees((viewData.cameraLon! as NSString).doubleValue)))
        self.motifPoint                   = MKMapPoint(CLLocationCoordinate2D(latitude: CLLocationDegrees((viewData.motifLat! as NSString).doubleValue), longitude: CLLocationDegrees((viewData.motifLon! as NSString).doubleValue)))
        
        let cameraName     : String       = viewData.cameraName ?? Constants.DEFAULT_CAMERA.name
        let sensorFormat   : Int          = Int(viewData.sensorFormat ?? "1") ?? 1
        self.camera                       = Camera(name: cameraName, sensorFormat: sensorFormat)
        
        let lensName       : String       = viewData.lensName ?? Constants.DEFAULT_LENS.name
        let minFocalLength : Double       = Double(viewData.minFocalLength!) ?? Constants.DEFAULT_LENS.minFocalLength
        let maxFocalLength : Double       = Double(viewData.maxFocalLength!) ?? Constants.DEFAULT_LENS.maxFocalLength
        let minAperture    : Double       = Double(viewData.minAperture!) ?? Constants.DEFAULT_LENS.minAperture
        let maxAperture    : Double       = Double(viewData.maxAperture!) ?? Constants.DEFAULT_LENS.maxAperture
    ...
    init(name: String, description: String, cameraLat: Double, cameraLon: Double, motifLat: Double, motifLon: Double, cameraName: String, sensorFormat: Int,
    lensName: String, minFocalLength: Double, maxFocalLength: Double, minAperture: Double, maxAperture: Double,
    focalLength: Double, aperture: Double, orientation: String, countryCode: String, originLat: Double, originLon: Double, mapWidth: Double, mapHeight: Double, tags: Int, equipment: Int, times: Int) {
        self.name          = name
        self.description   = description
        ...
        self.focalLength   = focalLength
        self.aperture      = aperture
        self.orientation   = CameraOrientation(rawValue: orientation)!
        self.countryCode   = countryCode
    ...
    public static func ==(lhs: PhotoView, rhs: PhotoView) -> Bool {
    ...
    public func clone() -> PhotoView {
        return PhotoView(name: self.name, description: self.description,
                    ...
                    countryCode: self.countryCode, originLat: self.mapRect.origin.coordinate.latitude, originLon: self.mapRect.origin.coordinate.longitude, mapWidth: self.mapRect.width, mapHeight: self.mapRect.height,
