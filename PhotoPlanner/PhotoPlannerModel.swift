...
public class PhotoPlannerModel {
    ...
    var lens                   : Lens              = Constants.DEFAULT_LENS {
        didSet {
            self.updateFoVTriangle(cameraPoint: MKMapPoint(self.cameraMarkerData?.coordinate ?? Constants.DEFAULT_LOCATION.coordinate), motifPoint: MKMapPoint(self.motifMarkerData?.coordinate ?? Constants.DEFAULT_LOCATION.coordinate), focalLength: self.lens.minFocalLength, aperture: self.lens.minAperture, sensorFormat: self.camera.sensorFormat, orientation: self.orientation)
            self.updateDoFTrapezoid(cameraPoint: MKMapPoint(self.cameraMarkerData?.coordinate ?? Constants.DEFAULT_LOCATION.coordinate), motifPoint: MKMapPoint(self.motifMarkerData?.coordinate ?? Constants.DEFAULT_LOCATION.coordinate), focalLength: self.lens.minFocalLength, aperture: self.lens.minAperture, sensorFormat: self.camera.sensorFormat, orientation: self.orientation)
        }
    }
    ...
    func updateFoVTriangle(cameraPoint: MKMapPoint, motifPoint: MKMapPoint, focalLength: Int, aperture: Double, sensorFormat: Int, orientation: CameraOrientation) -> Void {
        let distance : CLLocationDistance = cameraPoint.distance(to: motifPoint)
        if distance < 0.01 || distance > 9999 { return }
        
        do {
            self.fovData = try Helper.calc(camera: cameraPoint, motif: motifPoint, focalLengthInMM: Double(focalLength), aperture: aperture, sensorFormat: sensorFormat, orientation: orientation)
        ...
        // Update FoV Triangle
        let triangleCoordinates : [CLLocationCoordinate2D] = Helper.updateTriangle(camera: cameraPoint, motif: motifPoint, focalLengthInMM: Double(focalLength), aperture: aperture, sensorFormat: self.camera.sensorFormat, orientation: self.orientation)
        ...
        let minTriangleCoordinates : [CLLocationCoordinate2D] = Helper.updateTriangle(camera: cameraPoint, motif: motifPoint, focalLengthInMM: Double(self.lens.maxFocalLength), aperture: aperture, sensorFormat: self.camera.sensorFormat, orientation: self.orientation)
        ...
        let maxTriangleCoordinates : [CLLocationCoordinate2D] = Helper.updateTriangle(camera: cameraPoint, motif: motifPoint, focalLengthInMM: Double(self.lens.minFocalLength), aperture: aperture, sensorFormat: self.camera.sensorFormat, orientation: self.orientation)
    ...
    func updateDoFTrapezoid(cameraPoint: MKMapPoint, motifPoint: MKMapPoint, focalLength: Int, aperture: Double, sensorFormat: Int, orientation: CameraOrientation) -> Void {
        let distance : CLLocationDistance = cameraPoint.distance(to: motifPoint)
        if distance < 0.01 || distance > 9999 { return }
        
        // Update DoF Trapzoid
        let trapezoidCoordinates : [CLLocationCoordinate2D] = Helper.updateTrapezoid(camera: cameraPoint, motif: motifPoint, focalLengthInMM: Double(focalLength), aperture: aperture, sensorFormat: sensorFormat, orientation: orientation)
